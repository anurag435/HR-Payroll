    ...
    getEmployees()
      .then((res) => {
        const rawList = Array.isArray(res)
          ? res
          : res?.employees || res?.data?.employees || res?.data || res?.result || [];

        const normalized = rawList.map((emp) => ({
          ...emp,
          id: emp._id || emp.id,
          name: emp.name || `${emp.firstName || ""} ${emp.lastName || ""}`.trim() || "Unnamed",
      ...
      <div className="panel p-6">
        ...
        {!loading && view === "kanban" && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.length === 0 ? (
              <p className="text-sm text-text-muted col-span-full text-center py-6">No employees found.</p>
            ) : (
              filtered.map((emp) => {
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan="5" className="text-center py-6 text-sm text-text-muted">No employees found.</td>
                  </tr>
                ) : (
                  filtered.map((emp) => {
                    const empId = emp._id || emp.id;
                    return (
                      ...
                      >
                        ...
                        <td className="py-3 pr-4 text-text-secondary">{emp.workEmail || emp.email || "—"}</td>
